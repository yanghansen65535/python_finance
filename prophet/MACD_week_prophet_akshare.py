...
rmse = np.sqrt(
    sum((data_prophet["y"] - forecast["yhat"].head(train_len)) ** 2) / train_len)
print(rmse)
...
# 注意两个dataframe 的index得一致，所以先copy，再赋index
macd_prophet_plot['data'] = forecast['yhat']
...
if mode == 'verify':
    add_plot = [mpf.make_addplot(macdhist['y'].tail(data_macd_count), type='bar', panel=2, ylabel='MACD', color='darkslateblue'),
                ...
                mpf.make_addplot(macd_prophet_plot.tail(
                    data_macd_count), type='bar', panel=3, ylabel='MACD_p', color='darkslateblue')
                ]
...
if mode == 'forecast':
    ...
    index_append = pd.date_range(data_plot.index[-1] + timedelta(
        days=7), data_plot.index[-1]+timedelta(days=prophet_week*7), freq=period_type)
    data_plot = data_plot.append(pd.DataFrame(
        index=index_append, columns=data_plot.columns))
    macdhist = macdhist.append(pd.DataFrame(
        index=index_append, columns=macdhist.columns))
    macd = macd.append(pd.DataFrame(index=index_append, columns=macd.columns))
    macdsignal = macdsignal.append(pd.DataFrame(
        index=index_append, columns=macdsignal.columns))
    ...
    add_plot = [mpf.make_addplot(macdhist['y'].tail(data_macd_count+prophet_week), type='bar', panel=2, ylabel='week MACD', color='darkslateblue'),
                mpf.make_addplot(
                    macd.tail(data_macd_count+prophet_week), panel=2, color='orangered'),
                mpf.make_addplot(macdsignal.tail(
                    data_macd_count+prophet_week), panel=2, color='limegreen'),
                mpf.make_addplot(macd_prophet_plot.tail(data_macd_count+prophet_week),
                                 type='bar', panel=3, ylabel='week MACD_p', color='darkslateblue')
                ]
